...
/// 报告目录名
const REPORT_DIR: &str = ".devnpc-report";
/// 报告文件名
const REPORT_FILE: &str = "report.html";

/// 推送报告,返回报告文件的绝对路径
pub async fn publish(html: &str, target: &ReportTarget) -> Result<String> {
    match target {
        ReportTarget::None => {
...
fn get_report_path() -> Result<PathBuf> {
    ...
    Ok(cwd.join(REPORT_DIR).join(REPORT_FILE))
}

/// 获取报告目录路径
pub fn get_report_dir() -> Result<PathBuf> {
    let cwd = std::env::current_dir().map_err(|e| {
        crate::error::DevnpcError::Config(format!("无法获取当前目录: {e}"))
    })?;
    Ok(cwd.join(REPORT_DIR))
}
...
mod tests {
    ...
    async fn publish_artifact_creates_file() {
        ...
        run_in_temp_dir(|| async {
            let path = publish(html, &ReportTarget::Artifact).await.unwrap();
    ...
    async fn publish_pages_creates_file() {
        ...
        run_in_temp_dir(|| async {
            let path = publish(html, &ReportTarget::Pages).await.unwrap();
            let path_buf = Path::new(&path);
            let expected = Path::new(".devnpc-report").join("report.html");
            assert!(
                path_buf.ends_with(&expected),
    ...
    async fn publish_none_still_writes_file() {
        ...
        run_in_temp_dir(|| async {
            let path = publish(html, &ReportTarget::None).await.unwrap();
            let path_buf = Path::new(&path);
            let expected = Path::new(".devnpc-report").join("report.html");
            assert!(
                path_buf.ends_with(&expected),
    ...
    fn get_report_dir_returns_correct_path() {
        ...
        let report_dir = get_report_dir().unwrap();
